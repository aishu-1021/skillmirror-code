...
const AptitudeTest = () => {
  ...
  const buildImprovementPlan = (pct: number): string => {
    ...
    const urgencyMsg = pct < 50
      ? "Do not retake immediately. Work through each checklist below topic by topic, then retake."
      : "You are close to the pass mark. Focus only on your weak categories — you likely just need to fill a few specific gaps.";

    const failedCategories = Object.entries(stats).filter(([, d]) =>
      Math.round((d.correct / d.total) * 100) < 75
    );
    ...
    const categoryBlocks = failedCategories.map(([category, data]) => {
      const scorePct = Math.round((data.correct / data.total) * 100);
      const topics = topicMap[category] || [`Study ${category} fundamentals`, `Practice ${category} problems on IndiaBix`];
