...
import {
  ...
  LayoutDashboard, XCircle, CheckCircle, AlertCircle, BookOpen,
} from "lucide-react";
import { Chart, ArcElement, Tooltip, Legend, DoughnutController } from "chart.js";

Chart.register(ArcElement, Tooltip, Legend, DoughnutController);

export interface TechnicalResultsState {
  companyName: string;
  score: number;
...
const TechnicalResults = () => {
  ...
  const state = location.state as TechnicalResultsState;
  ...
  const { companyName, score, totalQuestions, percentage, passed, interviewUnlocked, categoryStats, wrongAnswers, resources } = state;
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-40">
        <div className="container mx-auto px-6 py-3.5 flex items-center justify-between">
          <Link to="/dashboard" className="flex items-center gap-2.5">
            <div className="h-8 w-8 rounded-lg bg-gradient-to-br from-indigo-600 to-blue-500 flex items-center justify-center">
          ...
          <div className="hidden sm:flex items-center gap-2 text-sm font-medium">
            ...
            <span className="text-slate-600">Technical Results</span>
          </div>
          <div className="flex gap-2">
            ...
            <Button size="sm" onClick={() => navigate(`/technical-round/${encodeURIComponent(companyName)}`)}>
      ...
      <div className="container mx-auto px-4 py-8 max-w-5xl space-y-5">
        ...
        <div className={`rounded-2xl overflow-hidden transition-all duration-700 ${animateScore ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
          <div className={`relative p-8 ${passed ? "bg-gradient-to-br from-indigo-700 via-indigo-600 to-blue-600" : "bg-gradient-to-br from-slate-700 via-slate-600 to-slate-500"}`}>
            ...
            <div className="relative flex flex-col md:flex-row items-center gap-8">
              ...
              <div className="flex-1 text-center md:text-left">
                <p className="text-indigo-200 text-xs font-semibold uppercase tracking-widest mb-2">
                  Technical Round · {companyName}
                </p>
                <h1 className="text-3xl font-black text-white mb-2 leading-tight">
                  {passed
                    ? interviewUnlocked ? "Interview Round Unlocked" : "Technical Round Cleared"
                    : "Keep Going"}
                </h1>
                <p className="text-white/60 text-sm leading-relaxed max-w-sm">
                  {passed
                    ? "You've qualified for the next round. A detailed report has been sent to your email."
                    : "Review your weak areas below, practice, and try again. You're getting closer."}
                </p>
              </div>
        ...
        {resources.length > 0 && (
          <Card className="border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
              {resources.map((r, i) => (
                <div key={i} className="rounded-xl border border-slate-200 bg-white p-4 hover:border-indigo-200 hover:shadow-sm transition-all">
        ...
        <div className="flex gap-3 justify-center pb-10">
          ...
          <Button size="lg" onClick={() => navigate(`/technical-round/${encodeURIComponent(companyName)}`)}>
